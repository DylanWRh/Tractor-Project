...
# return Banking
def checkBanker(repo, level, currplayer, banking, first_round): 
...
# history: other players' moves in the current round: list[list]
    ...
    if len(history) == 0: # The first move in a round
        ...
        typoker = checkPokerType(poker, currplayer, level)
        if typoker == "suspect":
            outpok_s, ilcnt = checkThrow(poker, own, currplayer, level, major, True)
            if ilcnt > 0:
        ...
        # 该情形下的非法行动：(1) 有可以应手的牌型但贴牌或用主牌毙 (2) 贴牌不当(有同花不贴/拖拉机有对子不贴)
            if checkRes(history[0], own[currplayer], level): #(1) 有应手但贴牌或毙
                ...
                if hist[0][0] in Major:
                    ...
                    suit = hist[0][0][0]
                    suit_pok = [p for p in pok if p[0] == suit and p not in Major]
...
def checkWinner(history, currplayer, level, major, banker):
    histo = history + []
    ...
    if tyfirst == "suspect": # 甩牌
        ...
        for i in range(1,4):
            ...
            for j in range(len(hist[i])):
                if hist[i][j] not in Major:
                    isMajor = False
                    break
            if not isMajor: # 副牌压主牌，算了吧
                continue
            if win_move[0] not in Major and isMajor: # 主牌压副牌，必须的
                win_move = hist[i]
                win_seq = i
...
def main(full_input):
    ...
    try:
        if lenLog == 0: # 刚开局
            ...
            if "first_round" not in full_input["initdata"]:
                globalInfo["first_round"] = True
            else:
                globalInfo["first_round"] = full_input["initdata"]["first_round"]
            
            initdata["first_round"] = globalInfo["first_round"]
            ...
            if len(repo) > 0:
                ...
                eventpoker = repo
                new_ = True
                newbanking = checkBanker(repo, level, currplayer, banking, globalInfo["first_round"])
            else: # 不报就不管
                eventpoker = []
                new_ = False
                newbanking = banking
            ...
            if lenLog < 200: # 给下一名玩家发牌
                ...
                printContent["global"] = globalInfo
                if lenLog < 8:
                    printContent["playerpos"] = nextplayer
                ...
                if newbanking["major"] == "":
                    newbanking["major"] = suitset[random.randint(0, 3)]
                globalInfo["banking"] = newbanking
            ...
            for turn_id in range(204, lenLog - 1, 2): # 还原本轮手牌（刚刚接收到的一回合信息不处理）
            ...
            if len(new_history) == 4: # 本回合为该轮最后一次出牌
                winner = checkWinner(history[1], currplayer, full_input["initdata"]["level"], major, banker)
                nextplayer = winner
                ...
                if len(new_alloc[currplayer]) == 0: # 本局结束
                    # 扣底
                    if checkPokerType(history[1][0], (currplayer-3)%4, full_input["initdata"]["level"]) != "suspect":
                        mult = len(history[1][0])
                    else:
                        divided, _ = checkThrow(history[1][0], [[]], (currplayer-3)%4, full_input["initdata"]["level"], major, check=False)
            ...
            if get_score != 0: # 非终止回合出现分数变动，说明甩牌失败
                displayContent["punished"] = True
            new_score = old_score + get_score
